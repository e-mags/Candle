...
{
    static QRegularExpression _e_re("[Ff]([0-9.]+)");
    auto _d_re = _e_re.globalMatch(command);
    if (_d_re.hasNext()) {
        auto re = _d_re.next();
        command.replace(re.capturedStart(0), re.capturedLength(0), QString("F%1").arg(re.captured(1).toDouble() / 100 * speed));

        if (original) *original = re.captured(1).toDouble();
    }
...
QString GcodePreprocessorUtils::truncateDecimals(int length, QString command)
{
    ...
    while ((pos = command.indexOf(re, pos, &match)) != -1)
    {
